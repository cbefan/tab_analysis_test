...
print('calculated Rgs')

    

# CALCULATE ANISOAPS
hypers = set_anisoap_hypers(lmax=lmax,nmax=nmax,cutoff_radius=cutoff_radius,gaussian_width=gaussian_width)
...
for job in project:
    ...
    for t in t_frames:
        ...
        frames.append(frame_to_ellipsoids(frame,scrungus))
    asoap = anisaponify_frame(frames,calculator,scrungus).tolist()
    asoap_dic[job] = asoap
    frames_dic[job] = frames

with open(f'{write_directory}/asoap.pkl','wb') as f:
    pickle.dump(asoap_dic,f)

with open(f'{write_directory}/asoap_frames.pkl','wb') as f:
    pickle.dump(frames_dic,f)

print('calculated anisoap stuff')
...
layers = []
qnorms = []
for job in asoap_dic:
    asoaps.extend(asoap_dic[job])
    N_job = len(asoap_dic[job])
...
scaling.fit(pd.DataFrame(asoaps))

features = scaling.transform(asoaps)

pcovr = PCovR(mixing=alpha,n_components=n_components)
pcovr.fit(features,layers)
pcovr_dic = {}
for job in asoap_dic:
    pcovr_dic[job] = pcovr.transform(asoap_dic[job])

#latent_features = pcovr.transform(features)

with open(f'{write_directory}/features.pkl','wb') as f:
    pickle.dump(pcovr_dic,f)

print('calculated pcovr stuff')
